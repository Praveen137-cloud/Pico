...
const correctSfx = new Audio('https://assets.mixkit.co/active_storage/sfx/1435/1435-preview.mp3');
...
const Lesson = () => {
  ...
  const [sassyOverride, setSassyOverride] = useState(null);
  ...
  // Voice Command (Prompt Wars VIP feature)
  const startListening = () => {
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SpeechRecognition) {
       alert("Voice tracking offline in this browser.");
       return;
    }
    ...
    recognition.onresult = (event) => {
       ...
       if (unit.lessons[currentLessonIndex].type === 'code_fill_in') {
          setUserCode(prev => prev + ' // ' + transcript);
       } else if (unit.lessons[currentLessonIndex].type === 'programming_board') {
          alert('You said: ' + transcript);
       } else {
          setAiHint(`🦜 Pico (Voice Mode): You asked about "${transcript}"! Keep pushing!`);
       }
    };
  ...
  useEffect(() => {
    if (!unit || feedback) return;
    
    if (timeLeft > 0) {
      const timer = setTimeout(() => setTimeLeft(prev => prev - 1), 1000);
      return () => clearTimeout(timer);
    }
  }, [timeLeft, feedback, unit]);
...
const sillyErrors = [
  "Pathetic attempt. My grandmother's toaster compiles better.",
  "Critical logic failure. Were you typing with your elbows?",
  "Access Denied. You call that an algorithm?",
  "I've seen syntax errors that made more sense."
];
  ...
  const checkAnswer = () => {
    ...
    if (currentLesson.type === 'multiple_choice') {
      if (normalizeAnswer(currentLesson.correctAnswer) === normalizeAnswer(selectedOption)) {
        ...
        setFeedback('wrong');
        setSassyOverride(sillyErrors[Math.floor(Math.random() * sillyErrors.length)]);
        playSound(false);
      }
    ...
    } else if (currentLesson.type === 'code_fill_in') {
      const expectedCode = currentLesson.codeSnippet.replace('___', currentLesson.correctAnswer);
      if (normalizeAnswer(expectedCode) === normalizeAnswer(userCode)) {
        setFeedback('correct');
        setLastTestResult(null);
        playSound(true);
      } else {
        setFeedback('wrong');
        setLastTestResult({ expected: expectedCode, actual: userCode || "[EMPTY]" });
        setSassyOverride(sillyErrors[Math.floor(Math.random() * sillyErrors.length)]);
        playSound(false);
      }
    }
  ...
  const executeAndCheckCode = async () => {
    ...
    try {
      ...
      const execRes = await api.post('/api/execute', {
        ...
        code: userCode,
        language: currentLesson.language || 'c'
      });
  ...
  const handleNext = async () => {
    ...
    if (currentLessonIndex + 1 < unit.lessons.length) {
      ...
      try {
        // 🔄 SYNC & REFRESH before navigation to ensure Map reflects progress
        const res = await api.post('/api/unlock', { subjectId, sectionId, unitId });
        if (res.data.success) {
          if (res.data.user) setUser(res.data.user); // ⚡ INSTANT LOCAL SYNC
          await refreshUser?.(); // Also trigger full refresh just in case
          localStorage.removeItem(`scroll_map_${sectionId}`);
        }
      } catch (err) {
        console.error('[Sync Fatal] Progress save failed.', err);
      }
  ...
  return (
    <div style={styles.page} className={shake ? 'animate-shake' : ''}>
      ...
      {showAccessGranted && <div className="access-granted-stamp">ACCESS GRANTED</div>}
      {feedback === 'correct' && <div className="victory-glow" />}
      {shake && <div className="impact-flash animate-shake-extreme" />}
      ...
      <div style={styles.content} className="smooth-scroll">
        ...
        {currentLesson.type !== 'teaching' && (
          <div style={styles.mascotArea} className="bounce-in">
            <div className="mascot-with-bubble">
              ...
              <div className={`duo-speech-bubble glass-panel ${feedback === 'correct' ? 'correct-celebration' : 'pop-in'}`}>
                ...
                <div style={{display: 'flex', gap: 10, marginTop: 10, justifyContent: 'center'}}>
                  ...
                  <button 
                    className={`tts-btn-mini hover-scale ${isListening ? 'listening-pulse' : ''}`}
                    onClick={startListening}
                    style={isListening ? { backgroundColor: '#FF003C', color: '#fff', borderColor: '#FF003C' } : {}}
                  >
            ...
            {feedback === 'wrong' && (
              <div className="explanation-bubble shake-in" style={ sassyOverride ? { backgroundColor: 'rgba(255,0,0,0.1)', border: '1px solid #FF003C', color: '#FF003C' } : {}}>
                <span style={{marginRight: 8}}>💡</span>
                {sassyOverride || currentLesson.explanation}
              </div>
            )}
          </div>
        )}
      ...
      <div style={{
        ...
        {!feedback && (currentLesson.type === 'code_fill_in') && (
          <button className="btn-chunky cyan" style={{ width: '100%' }} onClick={checkAnswer}>
            Run & Check
          </button>
        )}

        {!feedback && (currentLesson.type === 'code_problem' || currentLesson.type === 'programming_board') && (
          <div style={{ display: 'flex', gap: '12px', width: '100%' }}>
            <button 
              ...
              onClick={async () => {
                setIsCompiling(true);
                try {
                  const res = await api.post('/api/run', { code: userCode, language: currentLesson.language || 'c' });
                  setCodeOutput(res.data.output || res.data.stderr);
                } catch (err) {
                  setCodeOutput("SYSTEM ERROR: UNABLE TO CONTACT COMPILER CORE.");
                } finally {
                  setIsCompiling(false);
                }
              }} 
            ...
            <button 
              ...
              style={{ flex: 1, backgroundColor: 'var(--theme-primary)', color: '#000' }} 
              onClick={executeAndCheckCode} 
              disabled={isCompiling}
            >
              {isCompiling ? 'Submitting...' : 'Submit Solution'}
            </button>
          </div>
        )}
        ...
        {feedback === 'correct' && (
          <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%'}}>
            ...
            <button className="btn-chunky cyan" style={{ width: 'auto', padding: '12px 40px', opacity: isSubmitting ? 0.7 : 1 }} onClick={handleNext} disabled={isSubmitting}>{isSubmitting ? 'SYNCING...' : 'CONTINUE'}</button>
          </div>
        )}
        ...
        {feedback === 'wrong' && (
          <div style={{display: 'flex', flexDirection: 'column', gap: '12px', width: '100%'}}>
            ...
            <button className="btn-chunky purple" style={{ width: '100%', marginTop: 8 }} onClick={handleNext} disabled={isSubmitting}>GOT IT</button>
          </div>
        )}
      </div>
    </div>
