  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        ...
        let data = res.data;
        if (!data || !data.branches || data.branches.length < 5) {
          throw new Error('Using fallback for demo');
        }
        setOptions(data);
      } catch (err) {
        console.warn("Using rich fallback options for Prompt Wars demo");
        setOptions({
          branches: ["Software Engineering", "Data Science", "Artificial Intelligence", "Cybersecurity", "Prompt Engineering"],
          interestsByBranch: {
            "Software Engineering": ["Frontend Architecture", "Backend Distributed Systems", "Full-Stack Web3"],
            "Data Science": ["Data Engineering", "Machine Learning", "Quantitative Analysis"],
            "Artificial Intelligence": ["LLM Training", "Computer Vision", "Autonomous Agents"],
            "Cybersecurity": ["Penetration Testing", "Security Architecture", "Cryptography"],
            "Prompt Engineering": ["Agentic Workflows", "Prompt Hacking", "Generative Art"]
          }
        });
      }
    };
    ...
    <div className="career-guidance-page">
      <div className="career-container">
        <header className="career-header">
          <button onClick={() => navigate('/home')} className="back-btn-elite">← BACK TO HQ</button>
